...
class Bytecodes: AllStatic {
 public:
  enum Code {
    ...
    _profile              ,          //  <- profiling instr

    // Include generated bytecode constants
#include "generated/bytecodes.generated.hpp"
 ...
 private:
  ...
  static u_long      _lengths       [number_of_codes];
  static Code        _java_code     [number_of_codes];
  static jchar       _flags         [(1<<BitsPerShort)*2]; // all second page for wide formats
 ...
 public:
  ...
  static int         length_for     (Code code)    { return is_valid(code) ? _lengths[code] & 0xFFFF : -1; }
  static int         wide_length_for(Code code)    { return is_valid(code) ? _lengths[code]  >> 16 : -1; }
  ...
  static int         flags          (int code, bool is_wide) {
    assert(code == (uint16_t)code, "must be a short");
    return _flags[code + (is_wide ? (1<<BitsPerShort) : 0)];
  }
